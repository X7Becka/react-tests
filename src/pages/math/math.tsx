...
export class Math extends React.PureComponent <{}, IState> {
    constructor(props: {}) {
        super(props);
        this.state = {
    ...
    get matrix() {
        ...
        const rows = twoDimensionalArray?.map((row, index) => {
            const rowItem = row.map((item, index) => {
                return (
                    <div key={index} className="math__matrix-item">{item}</div>
                )
            })
            return (
                <div key={index} className="math__matrix-row">
                    {rowItem}
                </div>
            )
        })
        ...
        return (
            <div className="math__matrix-wrapper">
                {rows}
            </div>
        )
    }
